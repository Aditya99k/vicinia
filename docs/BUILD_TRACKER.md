...
- **Done when:** all 4 role-specific flows work against the real backend (not mocked). ✅ Verified live against a real `start-infra.sh` stack (Playwright walkthrough, not just manual clicking): fresh signups for all 4 roles, real product search → cart → WALLET checkout → order placed and tracked through its status timeline, a merchant applying → admin-approving → going live → creating a real listing through the UI, and an admin approving from the same pending queue. Both light and dark themes screenshotted at every step.
  ...
  **Follow-up pass — real per-shop browsing, shop attribution end to end, live search, real photos.** Seeded the catalog with 10 real (Unsplash-hotlinked) product photos and onboarded 2 real demo merchants with live listings via the actual signup → apply → admin-approve → go-live → create-listing flow (not direct DB writes) to make the home page worth screenshotting. Building the "stores near you, with each store's own items" section surfaced a genuine, non-obvious backend fact: `ListingResponse.merchantId` (and `CartResponse.merchantId`) is the merchant's *owner user ID*, not merchant-service's own `Merchant.id` — the two are different UUIDs for the same store. `GET /api/merchants/nearby` returns the latter under its `id` field and — by deliberate design ("no owner/... internals") — never returned the former at all, making it impossible for any client to correlate "this nearby store" with "its listings." Fixed by adding `ownerUserId` to `MerchantSummaryResponse`: not a new information leak, since inventory-service already hands that exact value to any authenticated customer via every listing/cart response — the field was just missing from the one place that would have let the frontend use it for grouping.

  Also added: a debounced live search-suggestions dropdown (`SearchBox`, product-level, separate from the full `/search` results); a `useMerchantDirectory` hook + `ShopBanner` component so "which store is this order from" stays visible on cart, checkout, and order-detail, not just the home page; and a `useMerchantDirectory`-driven fix on the product page itself — clicking a specific store's item tile now carries that listing's ID through so "Add to cart" adds *that* store's offer, not just whichever merchant's listing the API happened to list first (found by deliberately testing the multi-item-same-store cart flow, not assumed to work). Also fixed a real dark-mode contrast bug, pre-existing but only now surfaced by screenshotting more of the app: several badge/tag/icon-chip styles paired `--brand-soft` (a light tint in light mode, a dark muddy tint in dark mode — same low-opacity yellow, different base) with `--brand-ink` (always near-black), making text nearly invisible in dark mode. Added a dedicated `--on-brand-soft` token (dark text in light mode, the bright brand yellow in dark mode) instead of reusing `--brand-ink`, which still needs to stay dark for text on the *solid* brand yellow elsewhere (buttons, avatars).
