...
class FullOrderLifecycleE2ETest {
    ...
    void checkoutThroughSettlement() throws Exception {
        ...
        await("merchant order task created from order.confirmed").atMost(Duration.ofSeconds(15)).untilAsserted(() -> {
        ...
        await("delivery task assigned via order.ready -> Redis GEO search").atMost(Duration.ofSeconds(15)).untilAsserted(() -> {
        ...
        // --- Settlement ---
        await("settlement entry created from order.delivered").atMost(Duration.ofSeconds(15)).untilAsserted(() -> {
